...
from XIAN_Info import rent_type, city_info
...
class Rent(object):
    ...
    def __init__(self):
        ...
        self.db['xian'].create_index('m_url', unique=True)  # 以m端链接为主键进行去重
                ...
                for dist, dist_py in info[2].items():  # 各区及其拼音
                    ...
                    if len(bc_list) > 0:
                        for bc_name in bc_list:
                            ...
                            while has_more:
                                try:
                                    url = 'https://app.api.lianjia.com/Rentplat/v1/house/list?city_id={}&condition={}'                                           '/rt{}&limit=30&offset={}&request_ts={}&scene=list'.format(info[0],
                                                                                                     bc_name,
                                                                                                     type_code,
                                                                                                     idx*30,
                                                                                                     int(time.time()))
